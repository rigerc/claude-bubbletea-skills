...
// phase tracks which stage the UI is in.
type phase int
...
// streamMsg is sent on each tick to deliver the next pending message.
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		m.width = msg.Width
		//grad, _ := banner.GradientByName("aurora")
		cfg := banner.Config{
			Text:          "TEST-APP",
			Font:          "basic",
			//Gradient:      &grad,
...
func (m model) View() tea.View {
	...
	// Banner — persists across all phases.
