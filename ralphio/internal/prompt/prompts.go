...
const PlanningPrompt = `Study @PRD.md  to understand the product requirements. Study @tasks.json to understand the plan so far.
Based on @PRD.md, update @tasks.json as a JSON array sorted by priority (1 = highest) of items yet to be implemented. Each task must follow the schema: id, title, description, priority, status ("pending"), retryCount (0), maxRetries, validationCommand.
...
2. After implementing, run the tests (validationCommand) for that unit of code.
